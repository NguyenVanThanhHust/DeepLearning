...
# Create a larger blotches of noise which look 
...
# paint the string in a random location the bounding box
# also uses a random font, slight ramdo rotation
def paint_text(text, w, h, rotate=False, ud=False, multi_fonts=False):
    ...
    with cairo.Context(surface) as context:
        ...
        if box[2] > (w - 2 * border_w_h[1]) or box[3] > (h - 2*border_w_h[0]):
            raise IOError(('Could not fit the string into image.'
                            'Max char count is too large for given image width'))
        
        # teach the RNN translatational invariance by 
...
def shuffe_mat_or_lists(matric_list, stop_ind = None):
    ret = []
...
# Translation of characters to unique integer
def text_to_labesl(text):
    ret = []
    for char in text:
...
# Uses generator function to supply train/test with data
# Image rendering and text  are created on the fly 
# each time with random pertubations
class TextGenerator(keras.callbacks.Callback):
    ...
    def get_batch(self, index, size, train):
        ...
        source_str = []
        for i in range(size):
            # Mix in some blank inputs, for translatational invariance
