# -*- coding: utf-8 -*-
"""Triplet_loss_for_Recognition_Keras.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1YtVXM4prstJfr8vJGVfGT5aCVwxq5fDa
"""
...
# We need to prepare triplet batches for training
def get_batch_random(batch_size, mode = 'train'):
  """
  Create batch of anchor, positive and negative triplets with a complete random strategy

  Argument:
    batch_size --integer

  Returns:
    triplets --list containing 3 tensrofs A, P, N of shape (batch_size, w, h, c)
  """
  if mode == 'train':
    X = dataset_train
  else:
  ...
  for i in range(batch_size):
    # Pick one randome class for anchor
    anchor_class = np.random.randint(0, nb_classes)
    nb_sample_available_for_class_AP = X[anchor_class].shape[0]

    # Pick 2 random picture for this class, this is anchor and positive
    [idxA, idxP] = np.random.choice(nb_sample_available_for_class_AP, size=2, replace = False)

    # Pich another class for N, different from anchor_class
    negative_class = (anchor_class + np.random.randint(1, nb_classes)) % nb_classes
    nb_sample_available_for_class_N = X[negative_class].shape[0]
    idxN = np.random.randint(0, nb_sample_available_for_class_N)

    triplets[0][i,:,:,:] = X[anchor_class][idxA, :, :, :]
    triplets[1][i,:,:,:] = X[anchor_class][idxP, :, :, :]
    triplets[2][i,:,:,:] = X[negative_class][idxN, :, :, :]
  return triplets
