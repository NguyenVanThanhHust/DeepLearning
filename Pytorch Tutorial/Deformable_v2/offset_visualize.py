...
from example import MNISTClassifier
...
def plot_offsets(img, save_output, roi_x, roi_y):
    ...
    for offsets in save_output.outputs:
        offset_h, offset_w = offsets.shape[2:]
        resize_factor_h, resize_factor_w = input_img_h / offset_h, input_img_w/offset_w

        offsets_y = offsets[:, ::2]
        offsets_x = offsets[:, 1::2]

        grid_y = np.arange(0, offset_h)
        grid_x = np.arange(0, offset_w)
