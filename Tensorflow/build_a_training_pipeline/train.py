...
def main():
    ...
    # print out 2 outputs from our generator just to see that it works:
    iter = generator.get_next_pair()
    for i in range(2):
        print(next(iter))
    
    dataset = Dataset(generator)
    model_input = dataset.next_element
    model = Model(input)

    #train for 100 steps:
    with tf.Session() as sess:
        # sanity test: plot out the first resized and their label:
